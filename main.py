...
def check_guess(secret, guess):
    correct_pos = sum(1 for i, j in zip(secret, guess) if i == j)
    wrong_pos = sum((min(secret.count(n), guess.count(n)) for n in set(guess))) - correct_pos
    return correct_pos, wrong_pos

# /start command - starts single-player game
...
# Handling user guesses in single-player game
...
async def join(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await context.bot.send_message(
        chat_id=player1_id,
        text=f"Гравець <b>{player2_name}</b> приєднався до гри! Будь ласка, надішліть свій секретний 4-значний код за допомогою команди /setcode",
        parse_mode=ParseMode.HTML
    )
    await update.message.reply_text(
        f"Ви приєдналися до гри з <b>{player1_name}</b>! Будь ласка, надішліть свій секретний 4-значний код за допомогою команди /setcode",
        parse_mode=ParseMode.HTML
    )
...
async def setcode(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not context.args or len(context.args[0]) != 4 or not context.args[0].isdigit():
        await update.message.reply_text("Будь ласка, надішліть 4-значний код. Приклад: /setcode 1234")
        return
    ...
    if len(game_session['secrets']) == 2:
        ...
        opponent_name = (await context.bot.get_chat(opponent_id)).full_name
        ...
        for pid in game_session['players']:
            if pid == game_session['turn']:
                await context.bot.send_message(
                    chat_id=pid,
                    text=f"Гра розпочалася! Зараз ваш хід. Введіть свій здогад за допомогою команди /guess <число>",
                    parse_mode=ParseMode.HTML
                )
            else:
                await context.bot.send_message(
...
def main():
    ...
    application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, guess_single_player))
